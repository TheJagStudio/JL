(function ($) {
	...
	$(document).ready(function () {
		...
		$(".about-con").hover(function () {
			if (
				!$(this).hasClass("block-item-special") &&
				!$(this).hasClass("blockItemFirst")
			) {
		...
		if (typeof $.fn.waypoint == "function") {
			...
			$(
				".content-main-right .title-men, .content-main-right .desc-content, .content-main-right .join"
			).each(function (index) {
				$(this).waypoint(
					function () {
						$(this)
			...
			$(
				".content-main-left .title-men, .content-main-left .desc-content, .content-main-left .join"
			).each(function (index) {
				$(this).waypoint(
					function () {
						$(this)
			...
			$(".timetable .octember,.timetable .monday").addClass(
				"move_to_fadein_top"
			);
			$(
				".timetable .active .octember, .timetable .active .monday"
			).removeClass("move_to_fadein_top");
			$(".timetable").waypoint(
				function () {
					$(".title-time, .active .octember, .active .monday").each(
						function (index) {
							$(this)
								.delay(500 * index + 650)
								.animate(
			...
			$(".banner-text").waypoint(
				function () {
					$(
						".athlete-html, .athlete-welcome, .athlete-desc, .link-to"
					).each(function (index) {
						$(this)
							.delay(500 * index + 650)
							.animate(
			...
			$(
				".image-price-right, .image-price-left, .boxing-card-content, .yoga-card-content"
			).each(function (index) {
				$(this).waypoint(
					function () {
						$(this).animate(
			...
			$(".fit-strong-left").waypoint(
				function () {
					$(
						".fit-strong-text, .fit-strong-sub, .fit-strong-bottom"
					).each(function (index) {
						$(this)
							.delay(600 * index)
							.animate(
			...
			$(".product-store").waypoint(
				function () {
					$(".product-store .product-image-wrapper").each(function (
						index
					) {
						$(this)
							.delay(500 * index + 650)
	...
	function woocommerce_init() {
		...
		$(".quickview").on("click", function (e) {
			...
			$(el)
				...
				.find(".product-image")
				.append('<i class="quickviewloading fa fa-cog fa-spin"></i>');
			var effect = $(el).find("input").val();
			$.fn.custombox({
				url:
					woocommerce_params.ajax_url +
					"?action=load_product_quick_view&product_id=" +
					el.href.split("#")[1],
				...
				complete: function () {
					...
					$(".quickview-body .woocommerce-main-image").click(
						function (e) {
							e.preventDefault();
						}
					);
					$(".quickview-body .owl-carousel .owl-item a").click(
						function (e) {
							e.preventDefault();
							if (
								$(".quickview-body .woocommerce-main-image img")
									.length == 2
							) {
								$(
									".quickview-body .woocommerce-main-image img:first"
								).remove();
							}
							$(
								".quickview-body .woocommerce-main-image img"
								...
								$(".quickview-body .woocommerce-main-image img")
									...
									.hide();
								$(
									".quickview-body .woocommerce-main-image img:last"
								).fadeIn();
							});
							$(".quickview-body .woocommerce-main-image").append(
								'<img class="attachment-shop_single wp-post-image" style="display:none;" src="' +
									this.href +
									'" alt="">'
							);
						}
	...
	if ($(".variations_button").length) {
		$(".yith-wcwl-add-to-wishlist .link-wishlist").appendTo(
			".variations_button"
		);
	} else {
		$(".yith-wcwl-add-to-wishlist .link-wishlist").appendTo(
			".add-to-box form.cart"
		);
	}
	...
	$(function () {
		...
		$(".mainnav li a,.nav-menu li a,.page-scroll a").bind(
			"click",
			function (event) {
				...
				if (anchor.indexOf("#") >= 0 && $(anchor).length) {
					...
					if (
						$(".nav-menu:first").attr("id").indexOf("onepage") > 0
					) {
						window.location.href =
							$(".header .logo a").attr("href") + anchor;
					}
				}
			}
		);
		...
		if (window.location.hash) {
			...
			jQuery(
				'.one-page .mainnav li a[href*="' + window.location.hash + '"]'
			)
				.parent()
			...
			$(".one-page .mainnav li a").each(function () {
				...
				if (anchor.indexOf("#") >= 0 && $(anchor).length) {
					...
					if (
						...
						$(".one-page .nav-menu li").removeClass(
							"selected active"
						);
	...
	$(document).ready(function () {
		...
		if (
			$(".wrapper").hasClass("welcome") ||
			$(".wrapper").hasClass("coming-soon") ||
		...
		$.ajax({
			type: "GET",
			url:
				athleteCfg.baseUrl + "/wp-content/themes/athlete/css/color.css",
			dataType: "html",
	...
	function panelBindEvents() {
		...
		$(".sample-setting button").click(function () {
			if (!$(this).hasClass("active")) {
				...
				$(this).addClass("active");
				var newColorSetting = colorSetting.replace(
					/#ef791f/g,
					this.value
				);
		...
		if (defaultSetting) {
			...
			defaultSetting = {
				layout: $(".layout-setting button.active").val()
					? $(".layout-setting button.active").val()
					: "",
				mainColor: $(".sample-setting button.active").val()
					? $(".sample-setting button.active").val()
					: "",
				bgColor: $(".background-setting button.active").val()
					? $(".background-setting button.active").val()
					: "",
			};
			setCookie(
				"layoutsetting-default",
	...
	$(".main-contact-form").submit(function (e) {
		...
		if (
			email.length &&
			(email.val() == "" || !isValidEmailAddress(email.val()))
		) {
			email.focus();
			email.select();
			validate = false;
		...
		var captcha = $(".captcha-view", el).data("value"),
			input_val = $(".captcha", el).val();
	...
	$(".main-contact-form input,.main-contact-form textarea").focus(
		function () {
